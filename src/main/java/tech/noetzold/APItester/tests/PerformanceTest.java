...
public class PerformanceTest {
    ...
    public PerformanceTest(TestDeleteRequisition testDeleteRequisition) {
    ...
    public List<Result> runPutTests(int numTests, int numRequestsPerTest, Map<String,Object> body, Map<String, String> headers) {
        List<Result> results = new ArrayList<>();

        for (int i = 0; i < numTests; i++) {
            Result result = runPutTest(numRequestsPerTest, body, headers);
            results.add(result);
        }
    ...
    public Result runPutTest(int numRequests, Map<String, Object> body, Map<String, String> headers) {
        Response response = RestAssured.given()
                .headers(headers)
                .body(body)
                .when()
    ...
    public List<Result> runDeleteTests(int numTests, int numRequestsPerTest, Map<String, String> params, Map<String, String> headers) {
        List<Result> results = new ArrayList<>();

        for (int i = 0; i < numTests; i++) {
            Result result = runDeleteTest(numRequestsPerTest, params, headers);
            results.add(result);
        }
    ...
    private Result runDeleteTest(int numRequestsPerTest, Map<String, String> params, Map<String, String> headers) {
        Response response = RestAssured.given()
                .headers(headers)
                .params(params)
                .when()
